...
unsigned char uc_ComDHT11(void)
{
    ...
    for( i = 0; i < 8; i++ )
    {
        ...
        Delayus( 30 );
        if((PD_IDR & 0x10) == 0x10) // 高电平为1，低电平为0
        {
            ucComData |= ( 1 << ( 7 - i ) );
            while((PD_IDR & 0x10) == 0x10);
        }
...
void ReadTempAndHumi( float *pTempValue, float *pHumiValue )
{
    ...
    if(!((PD_IDR & 0x10) == 0x10))
    {
        while(!((PD_IDR & 0x10) == 0x10));   //80us的应答信号 DHT11 TO MCU
        while((PD_IDR & 0x10) == 0x10);      //80us的通知信号 DHT11 TO MCU
